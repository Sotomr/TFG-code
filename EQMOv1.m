function [costTotalGeneral, satisfFinal, bestIter] = EQMOv1(porcentajeOpt, porcentajeGroup, numIteracions, maxNoImprovement, numP, numD)
    fprintf('>> EQMO_V1: Peso Dinámico (adaptativo)\n');
    rng(145763);
...
function [EqFinal, tEntregaFinal, bestIter] = MultiObjetivo( ...
    ...
    asignacionGrupos(gruposSatisf) = 1;  % 1: búsqueda en satisfacció
    ...
    for it = 1:numIteracions
        ...
        % Ajust dinàmic: si la satisfacció es baixa, augmentar el pes de S.
        if satIter < 50
            alpha_eff = min(1, alpha_base + 0.2);
        else
            alpha_eff = alpha_base;
        end
        weightedSum = alpha_eff * satNorm + (1 - alpha_eff) * (1 - costNorm);
