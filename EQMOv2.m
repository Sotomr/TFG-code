function [costTotalGeneral, satisfFinal, bestIter] = EQMOv2(porcentajeOpt, porcentajeGroup, numIteracions, maxNoImprovement,numP, numD)
    fprintf('>> EQMO_V2: Combinación Multiplicativa\n');
    rng(145763);
    ...
    [EqFinal, tEntregaFinal, bestIter] = MultiObjetiuMultiplicatiu( ...
...
function [EqFinal, tEntregaFinal, bestIter] = MultiObjetiuMultiplicatiu( ...
    porcentajeOpt, porcentajeGroup, numIteracions, maxNoImprovement, ...
    numP, numD, K, preuD, CapBat0, vidaUtilD, distMax, costMant0, costE, ...
    costAss, baseConsumD, pesos, dist, ncicles, alfa, costBat, tvol, mD, vyD, ...
    horesVolAny, BateriaRestant, costFixDron, costExternalitzacio, preuE, ...
    ...
    for it = 1:numIteracions
        ...
        % Combinació multiplicativa:
        weightedProduct = (satNorm^alpha_base) * ((1 - costNorm)^(1 - alpha_base));
        
        fprintf('Fin iter %d: Coste= %.2f, Satisf= %.2f, WeightedProd= %.4f\n', ...
            it, cTotal, satIter, weightedProduct);
        
        if weightedProduct > bestWeightedProduct
