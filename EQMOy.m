function [costTotalGeneral, satisfFinal, bestIter] = EQMOy(porcentajeOpt, porcentajeGroup, numIteracions, maxNoImprovement,numP, numD)
    fprintf('>> EQMO_VNS: Variable Neighborhood Search Enhanced\n');
    rng(145763);
    ...
    [EqFinal, tEntregaFinal, bestIter] = MultiObjetivo_VNS( ...
    ...
    [~, ~, costTotalGeneral] = CostFinal( ...
        ...
        costFixDron, costExternalitzacio, zeros(1, numD));
end


%% Funció MultiObjetctiu  Variable Neighborhood Search (VNS)
function [EqFinal, tEntregaFinal, bestIter] = MultiObjetivo_VNS( ...
    porcentajeOpt, porcentajeGroup, numIteracions, maxNoImprovement, ...
    numP, numD, K, ...
    preuD, CapBat0, vidaUtilD, distMax, costMant0, costE, ...
    costAss, baseConsumD, pesos, dist, ...
    ...
    currentGroupSize = grupTamany;
    
    grupos = dividirGrupos(numP, currentGroupSize);
    numGrupos = length(grupos);
    gruposTotal = numGrupos;
    ...
    asignacionGrupos(indicesGrupos(1:numGruposSatisf)) = 1;
    ...
    for it = 1:numIteracions
        ...
        % Esmodifca el amany del grup cada x iteracions sense millora
        if mod(it, ceil(numIteracions/10)) == 0 && noImprovementCounter >= ceil(maxNoImprovement/2)
            % Reduuir el tamany de grup
            currentGroupSize = max(1, currentGroupSize - 1);
 
            numPerturb = max(1, round(0.1 * gruposTotal));
            gruposAperturb = randperm(gruposTotal, numPerturb);
            for idx = gruposAperturb
                paqsPert = grupos{idx};

                for p = paqsPert
                    candidatos = find(dist(p) <= distMax);
                    if ~isempty(candidatos)
                        d_sel = candidatos(randi(length(candidatos)));
                        Eq(:, p) = 0;
                        Eq(d_sel, p) = 1;
 
                        tEntrega(p) = (tvol(d_sel) + (2 * dist(p) / vyD(d_sel))) * 60;
                    end
                end
            end
            fprintf('  -> Perturbació aplicada en %d grupo i  sajusta currentGroupSize a %d.\n', numPerturb, currentGroupSize);
            noImprovementCounter = 0;
        ...
        % Para a cada grup, aplicar la heurística corresponent
        for g = 1:numGrupos
            paqsG = grupos{g};
            if asignacionGrupos(g) == 1
                ...
                [EqGrupo, ~, ~, tEntregaGrupo] = NashDrons( ...
                    ...
                    currentGroupSize, maxEstq, BateriaRestant, TRecarregaMax, UmbralRecarrega, ...
        ...
        alphaPeso = (porcentajeOpt / 100);
        weightedSum = alphaPeso * satNorm + (1 - alphaPeso) * (1 - costNorm);
        
        fprintf('Fin it %d: Coste = %.2f, Satisf = %.2f, WeightedSum = %.4f\n', it, cTotal, satIter, weightedSum);
        
        % Actualizar la millor solució
        if weightedSum > bestWeightedSum
            ...
            bestIter = it;
            noImprovementCounter = 0;
            fprintf('  -> Mejor solución en it %d.\n', it);
        else
...
function grupos = dividirGrupos(numP, tamanyGrupo)
    grupos = {};
    ini = 1;
    while ini <= numP
        fin = min(ini + tamanyGrupo - 1, numP);
...
function [EqGrupo, iter, costTRec, tEntregaGrupo] = NashDrons(...
    ...
    tvol, mD, vyD, horesVolAny, costAss, K, costFixDron, costExternalitzacio, preuE, ...
